...
class LiteLLMAIHandler(BaseAiHandler):
    ...
    def __init__(self):
        ...
        if get_settings().get("HUGGINGFACE.API_BASE", None) and 'huggingface' in get_settings().get("config.model", ""):
            litellm.api_base = get_settings().huggingface.api_base
            self.api_base = get_settings().huggingface.api_base
        if get_settings().get("OLLAMA.API_BASE", None) and 'ollama' in get_settings().get("config.model", ""):
